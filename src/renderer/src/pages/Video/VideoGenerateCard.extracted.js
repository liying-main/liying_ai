function VideoGenerateCard() {
  ...
  const [isUploadingVideo, setIsUploadingVideo] = reactExports.useState(false);
  const [videoUploadText, setVideoUploadText] = reactExports.useState("上传");
  ...
  const generateVideoIntervalRef = reactExports.useRef(null);
  const [isVideoQueuing, setIsVideoQueuing] = reactExports.useState(false);
  const isVideoQueueingRef = reactExports.useRef(false);
  ...
  const handleGenerateVideo = async () => {
    ...
    try {
      ...
      const fileName = urlFileName?.endsWith(".mp4")
        ? urlFileName
        : `generated_video_${/* @__PURE__ */ new Date().toISOString().replace(/[:.]/g, "-")}.mp4`;
...
function BgmGroupedPreviewSelect(props) {
  ...
  const [menuView, setMenuView] = reactExports.useState("categories");
  ...
  const flatAll = reactExports.useMemo(
...
function BgmUploadCategoryModal(props) {
  ...
  const [extraCategories, setExtraCategories] = reactExports.useState([]);
  const [newNameDraft, setNewNameDraft] = reactExports.useState("");
  const listRef = reactExports.useRef(null);
  const allChoices = reactExports.useMemo(() => {
...
function getUploadCategoryCandidates(allBgms) {
  ...
  const merged = /* @__PURE__ */ new Set([...presets, ...fromData]);
...
function ThemeTemplatePreviewCard({
  ...
  const staticRenderedRef = reactExports.useRef(false);
  ...
  const startAnimation = reactExports.useCallback(() => {
...
function SmartCutTimeline({
  ...
  const [mixDraggingOver, setMixDraggingOver] = reactExports.useState(false);
  const [mixDropPreviewTime, setMixDropPreviewTime] =
    reactExports.useState(null);
  const [mixDropPreviewDur, setMixDropPreviewDur] = reactExports.useState(5);
  const mixTrackRef = reactExports.useRef(null);
  const hasMixTrack = safeMixSegments.length > 0 || mixDraggingOver;
  const [pipDraggingOver, setPipDraggingOver] = reactExports.useState(false);
  const [pipDropPreviewTime, setPipDropPreviewTime] =
    reactExports.useState(null);
  const [pipDropPreviewDur, setPipDropPreviewDur] = reactExports.useState(5);
  const pipTrackRef = reactExports.useRef(null);
  const hasPipTrack = safePipSegments.length > 0 || pipDraggingOver;
  const [pxPerSecond, setPxPerSecond] =
    reactExports.useState(BASE_PX_PER_SECOND);
  reactExports.useEffect(() => {
    setPxPerSecond(BASE_PX_PER_SECOND);
  }, [safeDuration]);
  const rightScrollRef = reactExports.useRef(null);
  const rulerRef = reactExports.useRef(null);
  const [rulerRect, setRulerRect] = reactExports.useState({
    left: 0,
    width: 1,
  });
  const updateRulerRect = reactExports.useCallback(() => {
  ...
  const timeToX = reactExports.useCallback(
    (t) => {
  ...
  const xToTime = reactExports.useCallback(
    (clientX) => {
  ...
  const zoomReset = reactExports.useCallback(() => {
    setPxPerSecond(BASE_PX_PER_SECOND);
  }, []);
  ...
  const [playheadDragging, setPlayheadDragging] = reactExports.useState(false);
  const handlePlayheadMouseDown = reactExports.useCallback((e) => {
    e.preventDefault();
    e.stopPropagation();
  ...
  const handleBarMouseDown = reactExports.useCallback((e, seg, mode) => {
    e.preventDefault();
    e.stopPropagation();
  ...
  const [subtitleDragState, setSubtitleDragState] = reactExports.useState(null);
  const handleSubtitleBarMouseDown = reactExports.useCallback(
    (e, index, seg, mode) => {
  ...
  const [mixDragState, setMixDragState] = reactExports.useState(null);
  const handleMixBarMouseDown = reactExports.useCallback(
    (e, mix, mode) => {
  ...
  const [pipDragState, setPipDragState] = reactExports.useState(null);
  const handlePipBarMouseDown = reactExports.useCallback(
    (e, pip, mode) => {
...
const thumbUrlCacheByPath = /* @__PURE__ */ new Map();
const thumbPromiseByPath = /* @__PURE__ */ new Map();
...
function LocalVideoFirstFrameThumb({ videoPath }) {
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("svg", {
    ...
    children: [
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsx("path", { d: "M10 9l5 3-5 3V9z" }),
    ],
  });
}
const AlignVTopIcon = () =>
...
function SmartCutResourcePanel({
  ...
  const [showAutoAddCategoryDialog, setShowAutoAddCategoryDialog] =
    reactExports.useState(false);
  const [autoAddCategory, setAutoAddCategory] = reactExports.useState("");
  const [autoAddSimThreshold, setAutoAddSimThreshold] =
    reactExports.useState(0.45);
  const mixCategories = reactExports.useMemo(() => {
  ...
  const mixResourcesWithVector = reactExports.useMemo(
    () =>
      mixResources.filter(
  ...
  const [autoAddingMix, setAutoAddingMix] = reactExports.useState(false);
  ...
  const handleAutoAddMixSegments = reactExports.useCallback(async () => {
...
请输出适合插入混剪画面的句段序号（英文逗号分隔），数量不宜过多，不准超过字幕句段数量的一半（例如我给你的字幕列表是2，你返回的句段数量不能超时），只选真正适合的几句：`;
  ...
  const [localMainFont, setLocalMainFont] = reactExports.useState("黑体");
  const [localMainFontSize, setLocalMainFontSize] = reactExports.useState("");
  const [localMainTop, setLocalMainTop] = reactExports.useState("");
  const [localMainColor, setLocalMainColor] = reactExports.useState("#ffffff");
  ...
  const [localMainShadowBlur, setLocalMainShadowBlur] =
    reactExports.useState(0);
  const [localSubFont, setLocalSubFont] = reactExports.useState("黑体");
  const [localSubFontSize, setLocalSubFontSize] = reactExports.useState("");
  const [localSubTop, setLocalSubTop] = reactExports.useState("");
  const [localSubColor, setLocalSubColor] = reactExports.useState("#ffffff");
  ...
  const [localSubBgColor, setLocalSubBgColor] =
    reactExports.useState("#000000");
  const [localSubBgAlpha, setLocalSubBgAlpha] = reactExports.useState(100);
  ...
  const [localSubShadowOffsetY, setLocalSubShadowOffsetY] =
    reactExports.useState(2);
  const [localSubShadowBlur, setLocalSubShadowBlur] = reactExports.useState(0);
  const [localMainAlignV, setLocalMainAlignV] = reactExports.useState("top");
  const [localMainAlignH, setLocalMainAlignH] = reactExports.useState("center");
  const [localSubAlignV, setLocalSubAlignV] = reactExports.useState("top");
  const [localSubAlignH, setLocalSubAlignH] = reactExports.useState("center");
  reactExports.useEffect(() => {
  ...
  const [activeBgmFolder, setActiveBgmFolder] = reactExports.useState("");
  const [selectedBgmId, setSelectedBgmId] = reactExports.useState("");
  const [localBgmVolume, setLocalBgmVolume] = reactExports.useState(
    Math.round(DEFAULT_BGM_CARD_MUSIC_VOLUME * 100),
  );
  const [localVoiceVolume, setLocalVoiceVolume] = reactExports.useState(
    Math.round(DEFAULT_BGM_CARD_VOICE_VOLUME * 100),
  );
  const bgmUploadInputRef = reactExports.useRef(null);
  const pendingBgmUploadCategoryRef = reactExports.useRef("");
  ...
  const smartCutUploadCategories = reactExports.useMemo(
    () =>
  ...
  const allSmartCutBgms = reactExports.useMemo(
    () => [...localUploadedBgms, ...localBuiltinBgms],
    [localUploadedBgms, localBuiltinBgms],
  );
  const bgmFolders = reactExports.useMemo(() => {
  ...
  const currentBgmList = reactExports.useMemo(
    () =>
...
function SmartCutMixUploadWizard({ onCancel, onFinished }) {
  ...
  const [categories, setCategories] = reactExports.useState(["默认分类"]);
  const [selectedCategory, setSelectedCategory] =
    reactExports.useState("默认分类");
  ...
  reactExports.useEffect(() => {
    window.api.loadMixResourcesConfig().then((res) => {
      ...
      const fromConfig = /* @__PURE__ */ new Set();
      items.forEach((item) => {
  ...
  const [recognizing, setRecognizing] = reactExports.useState(false);
  const [recognizeError, setRecognizeError] = reactExports.useState(null);
  ...
  const [vectorizing, setVectorizing] = reactExports.useState(false);
  const [vectorError, setVectorError] = reactExports.useState(null);
...
function CompositePreview(props) {
  ...
  const bgmAudioRef = reactExports.useRef(null);
  const mixVideoRefs = reactExports.useRef({});
  const pipVideoRefs = reactExports.useRef({});
  const subtitleCanvasRef = reactExports.useRef(null);
  const [overlayRect, setOverlayRect] = reactExports.useState({
    x: 0,
    y: 0,
    w: 0,
    h: 0,
  });
  const [resourceUrls, setResourceUrls] = reactExports.useState({});
  const [bgmUrl, setBgmUrl] = reactExports.useState(null);
  const [mainVidDrag, setMainVidDrag] = reactExports.useState(null);
  const [pipDrag, setPipDrag] = reactExports.useState(null);
  const subDragRef = reactExports.useRef(null);
  reactExports.useEffect(() => {
  ...
  const syncBgm = reactExports.useCallback(
  ...
  const activeMixSeg = reactExports.useMemo(
    () =>
      mixSegments.find((s) => currentTime >= s.start && currentTime < s.end) ??
      null,
    [mixSegments, currentTime],
  );
  const activePipSegs = reactExports.useMemo(
    () =>
      pipSegments.filter((s) => currentTime >= s.start && currentTime < s.end),
    [pipSegments, currentTime],
  );
  const prevActiveMixIdRef = reactExports.useRef(null);
  const prevActivePipIdsRef = reactExports.useRef(/* @__PURE__ */ new Set());
  reactExports.useEffect(() => {
  ...
  const mixVideoIds = reactExports.useMemo(() => {
    const ids = [
  ...
  const mixImageIds = reactExports.useMemo(() => {
    const ids = [
      ...new Set(mixSegments.map((s) => s.mixResourceId).filter(Boolean)),
    ];
    return ids.filter((id) => {
  ...
  const handleSeeking = reactExports.useCallback(() => {
    const t = videoRef.current?.currentTime ?? 0;
    onVideoSeeking?.(t);
  ...
  const showTitle = reactExports.useMemo(() => {
  ...
  const currentSubLine = reactExports.useMemo(() => {
  ...
  const renderSubtitleCanvas = reactExports.useCallback(() => {
  ...
  const overlayToPercent = reactExports.useCallback((clientX, clientY) => {
  ...
  const [pipMaterialSizes, setPipMaterialSizes] = reactExports.useState({});
  const pipSizeRatio = reactExports.useMemo(() => {
  ...
  const [subtitleDragging, setSubtitleDragging] = reactExports.useState(false);
  reactExports.useEffect(() => {
    const handleMove = (e) => {
...
function SmartCutModal({ show, onClose }) {
  const showToast = useToast();
  const [audioDuration, setAudioDuration] = reactExports.useState(0);
  const [generatedVideoPreview, setGeneratedVideoPreview] =
    reactExports.useState("");
  const [mode, setMode] = reactExports.useState("editing");
  const [mixResources, setMixResources] = reactExports.useState([]);
  const [pipResources, setPipResources] = reactExports.useState([]);
  const [localMixSegments, setLocalMixSegments] = reactExports.useState([]);
  const [lastAppliedMixSegments, setLastAppliedMixSegments] =
    reactExports.useState(null);
  const [localPipSegments, setLocalPipSegments] = reactExports.useState([]);
  ...
  const [activeResourceType, setActiveResourceType] =
    reactExports.useState("mix");
  const [mixSubTab, setMixSubTab] = reactExports.useState("timeline");
  const [rightPanelTab, setRightPanelTab] = reactExports.useState("preview");
  ...
  const [selectedPipResourceId, setSelectedPipResourceId] =
    reactExports.useState(null);
  const [pipRect, setPipRect] = reactExports.useState(DEFAULT_PIP_RECT);
  const [mainVideoSize, setMainVideoSize] = reactExports.useState(null);
  const [pipVideoSize, setPipVideoSize] = reactExports.useState(null);
  const [mainVideoRect, setMainVideoRect] = reactExports.useState({
    x: 0,
    y: 0,
    width: 100,
    height: 100,
  ...
  const [mainVideoBgColor, setMainVideoBgColor] =
    reactExports.useState("#000000");
  const [mainVideoZIndex, setMainVideoZIndex] = reactExports.useState(0);
  ...
  const [snapThreshold, setSnapThreshold] = reactExports.useState(3);
  const [_videoFirstFrameUrl, setVideoFirstFrameUrl] =
    reactExports.useState(null);
  const [_pipFirstFrameUrl, setPipFirstFrameUrl] = reactExports.useState(null);
  const [localBuiltinBgms, setLocalBuiltinBgms] = reactExports.useState([]);
  const [localUploadedBgms, setLocalUploadedBgms] = reactExports.useState([]);
  ...
  const [localSubtitleEffectConfig, setLocalSubtitleEffectConfig] =
    reactExports.useState(null);
  const [localWhisperSegments, setLocalWhisperSegments] = reactExports.useState(
    [],
  );
  ...
  const [videoAlreadyHasSubtitle, setVideoAlreadyHasSubtitle] =
    reactExports.useState(false);
  const [initialMainTitle, setInitialMainTitle] = reactExports.useState("");
  const [initialSubTitle, setInitialSubTitle] = reactExports.useState("");
  ...
  const [lastAppliedBgmConfig, setLastAppliedBgmConfig] =
    reactExports.useState(null);
  const [titleSegmentRange, setTitleSegmentRange] = reactExports.useState(null);
  const [lastAppliedTitleSegmentRange, setLastAppliedTitleSegmentRange] =
    reactExports.useState(null);
  const [bgmSegmentRange, setBgmSegmentRange] = reactExports.useState(null);
  const [lastAppliedBgmSegmentRange, setLastAppliedBgmSegmentRange] =
    reactExports.useState(null);
  const [localPreviewUrl, setLocalPreviewUrl] = reactExports.useState(null);
  const [localVideoPath, setLocalVideoPath] = reactExports.useState(null);
  ...
  const [isWhisperGenerating, setIsWhisperGenerating] =
    reactExports.useState(false);
  const initialTitleConfigRef = reactExports.useRef(null);
  const initialSubtitleConfigRef = reactExports.useRef(null);
  const initialWhisperSegmentsRef = reactExports.useRef(null);
  const initialBgmConfigRef = reactExports.useRef(null);
  const initialVideoPathRef = reactExports.useRef(null);
  const lastAppliedVideoPathRef = reactExports.useRef(null);
  const [availableFonts, setAvailableFonts] = reactExports.useState(["黑体"]);
  const subtitleTextFromStoreRef = reactExports.useRef("");
  const whisperSegmentsFromStoreRef = reactExports.useRef([]);
  reactExports.useEffect(() => {
    if (show) {
  ...
  const pipSizeOwnerRef = reactExports.useRef(null);
  reactExports.useEffect(() => {
  ...
  const pipNormalizedRef = reactExports.useRef(null);
  reactExports.useEffect(() => {
  ...
  const handleRemoveMixResource = reactExports.useCallback((id) => {
  ...
  const handleAddPipSegment = reactExports.useCallback(
  ...
  const onSelectPipResource = reactExports.useCallback((id) => {
    setSelectedPipResourceId(id);
  }, []);
  const timelineCurrentTimeRef = reactExports.useRef(0);
  const handleDropPipResource = reactExports.useCallback(
    (resourceId, dropTime) => {
      if (dropTime != null) {
  ...
  const handleAddMixSegmentFromDrop = reactExports.useCallback(
    (resourceId, dropTime, centered) => {
  ...
  const handleDropMixResource = reactExports.useCallback(
    (resourceId, dropTime) => {
      if (dropTime != null) {
  ...
  const handleAddMixSegmentAtPlayhead = reactExports.useCallback(() => {
  ...
  const handleAddPipSegmentAtPlayhead = reactExports.useCallback(() => {
  ...
  const handleUpdatePipMaterial = reactExports.useCallback(() => {
    if (!selectedPipResourceId) return;
    setLocalPipSegments((prev) =>
  ...
  const autoSwitchToTextPreview = reactExports.useCallback(() => {
    setRightPanelTab("preview");
  }, []);
  const handleSubtitleEffectConfigChange = reactExports.useCallback(
    (config) => {
  ...
  const ensureSubtitleSeedText = reactExports.useCallback(() => {
  ...
  const fetchAndApplyWhisper = reactExports.useCallback(async () => {
  ...
  const handleAddTitleClick = reactExports.useCallback(async () => {
    const storeState = useVideoPageStore.getState();
  ...
  const handleAddSubtitleClick = reactExports.useCallback(async () => {
  ...
  const [isApplyingEffects, setIsApplyingEffects] =
    reactExports.useState(false);
  const lastMixPipBasePathRef = reactExports.useRef(null);
  const handleApplyEffects = reactExports.useCallback(async () => {
  ...
  const initialSegments = reactExports.useMemo(() => {
  ...
  const [segments, setSegments] = reactExports.useState(initialSegments);
  const [timelineCurrentTime, setTimelineCurrentTimeRaw] =
    reactExports.useState(0);
  const setTimelineCurrentTime = reactExports.useCallback((t) => {
  ...
  const lastSyncVideoTimeRef = reactExports.useRef(0);
  const lastSyncWallTimeRef = reactExports.useRef(0);
  ...
  const handleTimelineSeek = reactExports.useCallback(
    (time) => {
  ...
  const togglePreviewPlay = reactExports.useCallback(() => {
    ...
    children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", {
      ...
      children: [
        ...
        mode === "editing"
          ...
          : /* @__PURE__ */ jsxRuntimeExports.jsx(SmartCutMixUploadWizard, {
              onCancel: () => {
