...
import { VideoFirstFrameSelect } from './VideoSharedComponents'
...
export function VideoGenerateCard() {
  ...
  const _handleGenerateVideo_disabled = async () => {
                        ...
                        children: jsxRuntimeExports.jsx(
                          VideoFirstFrameSelect,
                          {
                            ...
                            options: [...uploadedVideos, ...builtinVideos].map(
                              (v) => ({ id: v.id, name: v.name, path: v.path, coverUrl: v.coverUrl || v.path }),
                            ),
                          },
                        ),
                      }),
              ...
              jsxRuntimeExports.jsxs("div", {
                ...
                children: [
                  jsxRuntimeExports.jsxs("button", {
                    ...
                    children: [
                      jsxRuntimeExports.jsxs("svg", { width: 16, height: 16, viewBox: "0 0 24 24", fill: "none", stroke: "currentColor", strokeWidth: 2, strokeLinecap: "round", strokeLinejoin: "round", children: [jsxRuntimeExports.jsx("polygon", { points: "23 7 16 12 23 17 23 7" }), jsxRuntimeExports.jsx("rect", { x: "1", y: "5", width: "15", height: "14", rx: "2", ry: "2" })] }),
                      isGeneratingVideo
                        ? isVideoQueuing
                          ? "排队中.."
    ...
    async (triggeredBy, skip) => {
      ...
      try {
        ...
        if (hasTitle && titleEffectConfig) {
          ...
          const _bStyles = store.builtinTitleStyles || [];
          const _bMatch = currentTitleStyle.id ? _bStyles.find((s) => s.id === currentTitleStyle.id) : null;
          const _mMain = { ...(_bMatch?.mainTitle || {}), ...(currentTitleStyle.mainTitle || {}) };
          const _mSub = currentTitleStyle.subTitle != null
            ? { ...(_bMatch?.subTitle || {}), ...currentTitleStyle.subTitle }
            : currentTitleStyle.subTitle;
          const combinedResult = await generateCombinedTitleImage({
            mainTitle: _mMain.font ? _mMain : {
              font: "黑体",
              fontSize: 48,
              fontWeight: 400,
              color: "#FFFFFF",
            ...
            subTitle: _mSub,
            hasSubTitle: (_bMatch?.hasSubTitle ?? currentTitleStyle.hasSubTitle) || false,
            mainTitleText,
            subTitleText,
            customPosX: titleEffectConfig.customPosX ?? null,
            customPosY: titleEffectConfig.customPosY ?? null,
          });
...
function isAllowedSmartcutMp4VideoFile(file) {
  if (!file || !file.name) return false;
  const ext = file.name.toLowerCase().split('.').pop();
  return ['mp4', 'mov', 'webm', 'avi', 'mkv', 'png', 'jpg', 'jpeg', 'gif', 'webp'].includes(ext);
}
function isImageFile(nameOrPath) {
  const lower = nameOrPath.toLowerCase();
  return /\.(jpg|jpeg|png)$/.test(lower);
}
const SMARTCUT_MIX_INITIAL_SEGMENT_SEC = 5;
function getSmartcutMixInitialSegmentDuration(item) {
  const path = item.path ?? "";
  const isImg = item.duration === Infinity || isImageFile(path);
  if (isImg) return SMARTCUT_MIX_INITIAL_SEGMENT_SEC;
  const d = item.duration;
  if (d != null && d > 0 && Number.isFinite(d)) {
    return d;
  }
  return SMARTCUT_MIX_INITIAL_SEGMENT_SEC;
}
...
function drawTitleBlockOnCanvas(ctx, text, config, canvasWidth, yOffset, forceCenterX) {
...
// Canvas-based scaled title rendering (ported from competitor v5.7.5)
function calcTextBlockHeight(ctx, text, config, scale) {
...
function drawScaledTextBlock(ctx, text, config, canvasWidth, scale, yOffset, forceCenterX) {
  ...
  if (forceCenterX != null) {
    ...
    centerX = canvasWidth * 0.05 + boxW / 2;
  } else if (alignH === 'right') {
    centerX = canvasWidth * 0.95 - boxW / 2;
  } else {
    centerX = canvasWidth / 2;
  }
...
function renderTitleConfigOnCanvas(ctx, config, canvasWidth, canvasHeight) {
  const scaleX = canvasWidth / 720;
  const scaleY = canvasHeight / 1280;
  const scale = Math.min(scaleX, scaleY);
  const mainConfig = config.mainTitle ?? {};
  const mainText = config.mainTitleText || '';
  if (!mainText) return;
  const alignV = mainConfig.alignV || 'top';
  const top = mainConfig.top ?? 100;
  let mainY;
  if (config.customPosY != null) {
    mainY = config.customPosY * scale;
  } else if (alignV === 'top') {
    mainY = top * scale;
  } else if (alignV === 'bottom') {
    mainY = canvasHeight - top * scale - calcTextBlockHeight(ctx, mainText, mainConfig, scale);
  } else {
    mainY = (canvasHeight - calcTextBlockHeight(ctx, mainText, mainConfig, scale)) / 2;
  }
  const forceCenterX = config.customPosX != null ? config.customPosX * scaleX : undefined;
  ctx.save();
  const result = drawScaledTextBlock(ctx, mainText, mainConfig, canvasWidth, scale, mainY, forceCenterX);
  if (config.hasSubTitle && config.subTitle && config.subTitleText) {
    const subConfig = config.subTitle;
    const subGap = subConfig.top ?? 0;
    const subY = mainY + result.totalHeight * scale + subGap * scale;
    drawScaledTextBlock(ctx, config.subTitleText, subConfig, canvasWidth, scale, subY, forceCenterX);
  }
  ctx.restore();
}
async function generateCombinedTitleImage(styleConfig) {
  ...
  let mainY;
  if (styleConfig.customPosY != null) {
    mainY = styleConfig.customPosY;
  } else if (alignV === 'top') {
    mainY = top;
  } else if (alignV === 'bottom') {
  ...
  ctx.save();
  const forceCenterX = styleConfig.customPosX != null ? styleConfig.customPosX : undefined;
  const result = drawTitleBlockOnCanvas(ctx, mainText, mainConfig, 720, mainY, forceCenterX);
  if (styleConfig.hasSubTitle && styleConfig.subTitle && styleConfig.subTitleText) {
    ...
    drawTitleBlockOnCanvas(ctx, styleConfig.subTitleText, subConfig, 720, subY, forceCenterX);
  }
  ctx.restore();
  return { dataUrl: canvas.toDataURL('image/png') };
}
async function generateTitleImage(text, config, _videoWidth = 720) {
  const canvas = document.createElement('canvas');
  const ctx = canvas.getContext('2d');
  if (!ctx) throw new Error('无法创建Canvas上下文');
  ...
  const canvasWidth = Math.ceil(maxW + padding * 2);
  const canvasHeight = Math.ceil(lines.length * lineHeight + padding * 2);
  canvas.width = canvasWidth;
  canvas.height = canvasHeight;
  drawTitleBlockOnCanvas(ctx, text, config, canvasWidth, 0);
  return { dataUrl: canvas.toDataURL('image/png'), height: canvas.height };
}
              ...
              children: [
                jsxRuntimeExports.jsx("button", {
                  ...
                  children: jsxRuntimeExports.jsx("svg", {
                    ...
                    "aria-hidden": "true",
                    focusable: "false",
                    children: <rect x="2" y="6.25" width="10" height="1.5" rx="0.75" fill="currentColor" />,
                  }),
                }),
                jsxRuntimeExports.jsx("button", {
                  type: "button",
                  ...
                  jsxRuntimeExports.jsx("button", {
                    ...
                    children: jsxRuntimeExports.jsxs("svg", {
                      ...
                      children: [
                        ...
                        <path d="M7 3.5V2.25M10.5 7H11.75M7 10.5V11.75M3.5 7H2.25" fill="none" stroke="currentColor" strokeWidth="1.25" strokeLinecap="round" />,
                      ],
                    }),
                  }),
              ],
...
const AlignVTopIcon = () =>
  jsxRuntimeExports.jsxs("svg", {
    ...
    children: [
      ...
      <rect x="2.5" y="3.5" width="3.5" height="8" rx="0.5" />,
      <rect x="8" y="3.5" width="3.5" height="6" rx="0.5" />,
    ],
  });
const AlignVMiddleIcon = () =>
  jsxRuntimeExports.jsxs("svg", {
    ...
    children: [
      ...
      <rect x="8" y="3" width="3.5" height="8" rx="0.5" />,
    ],
  });
const AlignVBottomIcon = () =>
  jsxRuntimeExports.jsxs("svg", {
    ...
    children: [
      ...
      <rect x="2.5" y="2.5" width="3.5" height="8" rx="0.5" />,
      <rect x="8" y="4.5" width="3.5" height="6" rx="0.5" />,
    ],
  });
const AlignHLeftIcon = () =>
  jsxRuntimeExports.jsxs("svg", {
    ...
    children: [
      ...
      <rect x="3.5" y="2.5" width="8" height="2.5" rx="0.5" />,
      <rect x="3.5" y="6.5" width="5.5" height="2.5" rx="0.5" />,
      <rect x="3.5" y="10" width="7" height="2" rx="0.5" />,
    ],
  });
const AlignHCenterIcon = () =>
  jsxRuntimeExports.jsxs("svg", {
    ...
    children: [
      ...
      <rect x="2" y="2.5" width="10" height="2.5" rx="0.5" />,
      <rect x="3.5" y="6.5" width="7" height="2.5" rx="0.5" />,
      <rect x="2.5" y="10" width="9" height="2" rx="0.5" />,
    ],
  });
const AlignHRightIcon = () =>
  jsxRuntimeExports.jsxs("svg", {
    ...
    children: [
      ...
      <rect x="2.5" y="2.5" width="8" height="2.5" rx="0.5" />,
      <rect x="5" y="6.5" width="5.5" height="2.5" rx="0.5" />,
      <rect x="3.5" y="10" width="7" height="2" rx="0.5" />,
    ],
  });
const VALIGN_OPTS = [
...
请输出适合插入混剪画面的句段序号（英文逗号分隔），数量不宜过多，不准超过字幕句段数量的一半（例如我给你的字幕列表是2，你返回的句段数量不能超时），只选真正适合的几句：`;
      ...
      for (const idx of selectedIndices) {
        ...
        for (const r of resourcesInCategory) {
          ...
          console.log(`${text} 与 ${r.name} 的相似度：${sim}`);
          if (sim > bestSim) {
            bestSim = sim;
            bestId = r.id;
          }
          ...
          children: jsxRuntimeExports.jsxs("div", {
            ...
            children: [
              jsxRuntimeExports.jsx("h2", {
                ...
                children: "选择用于智能添加的分类",
              }),
              jsxRuntimeExports.jsx("p", {
                style: { margin: "0 0 8px", fontSize: 13, color: "#666" },
                children: "智能添加只会在所选分类下的混剪素材中进行检索。",
              }),
              jsxRuntimeExports.jsx("div", {
...
async function applyTitleToVideo(inputPath, titleConfig, titleTimeRange) {
  ...
  const subTitleText = currentTitleStyle.hasSubTitle
    ...
    : void 0;
  const _builtinStyles = useVideoPageStore.getState().builtinTitleStyles || [];
  const _matchBuiltin = currentTitleStyle.id
    ? _builtinStyles.find((s) => s.id === currentTitleStyle.id)
    : null;
  const _mergedMain = { ...(_matchBuiltin?.mainTitle || {}), ...(currentTitleStyle.mainTitle || {}) };
  const _mergedSub = currentTitleStyle.subTitle != null
    ? { ...(_matchBuiltin?.subTitle || {}), ...currentTitleStyle.subTitle }
    : currentTitleStyle.subTitle;
  const combinedResult = await generateCombinedTitleImage({
    mainTitle: _mergedMain.font ? _mergedMain : {
      font: "黑体",
      fontSize: 48,
      fontWeight: 400,
      color: "#FFFFFF",
    ...
    subTitle: _mergedSub,
    hasSubTitle: (_matchBuiltin?.hasSubTitle ?? currentTitleStyle.hasSubTitle) || false,
    mainTitleText,
    subTitleText,
    customPosX: titleConfig.customPosX ?? null,
    customPosY: titleConfig.customPosY ?? null,
  });
  const titleResult = await window.api.addTitleToVideo(
    inputPath,
    combinedResult.dataUrl,
...
async function updateVideoGenerateHistorySnapshot() {
  try {
    ...
    const item = {
      ...
      generatedVideoPath: state.generatedVideoPath ?? '',
      ...
      mixSegments: state.mixSegments ?? [],
      pipSegments: state.pipSegments ?? [],
      pipRect: state.pipRect ?? null,
      mainVideoRect: state.mainVideoRect ?? { x: 0, y: 0, width: 100, height: 100 },
      bgmSegmentRange: state.bgmSegmentRange ?? null,
    };
    const list = await window.api?.getVideoGenerateHistory?.() || [];
    const arr = Array.isArray(list) ? list : [];
    arr.push(item);
    const trimmed = arr.length > HISTORY_MAX ? arr.slice(-HISTORY_MAX) : arr;
    await window.api?.setVideoGenerateHistory?.(trimmed);
    state.setVideoHistoryCurrentId?.(item.id);
    state.setVideoHistoryCurrentCreatedAt?.(item.createdAt);
  } catch (e) {
    console.error('Failed to update history snapshot:', e);
  }
}
...
function CompositePreview(props) {
  ...
  useEffect(() => {
    ...
    const onMouseDownCapture = (e) => {
      ...
      if (t.closest(".smartcut-title-drag-handle")) return;
      onSelectLayer(null);
    };
  ...
  useEffect(() => {
    ...
    if (!canvas) return;
    const dpr = window.devicePixelRatio || 1;
    const cw = Math.round(overlayRect.w * dpr);
    const ch = Math.round(overlayRect.h * dpr);
    if (cw <= 0 || ch <= 0) return;
    if (canvas.width !== cw || canvas.height !== ch) {
      canvas.width = cw;
      canvas.height = ch;
    ...
    if (!showTitle || !titleConfig?.style) return;
    const style = titleConfig.style;
    const _titleRenderConfig = {
      mainTitle: style.mainTitle ?? {},
      subTitle: style.hasSubTitle ? (style.subTitle ?? {}) : undefined,
      hasSubTitle: style.hasSubTitle || false,
      mainTitleText: titleConfig.mainTitleText || '',
      subTitleText: style.hasSubTitle ? (titleConfig.subTitleText || '') : undefined,
      customPosX: titleConfig.customPosX ?? null,
      customPosY: titleConfig.customPosY ?? null,
    };
    renderTitleConfigOnCanvas(ctx, _titleRenderConfig, cw, ch);
  }, [showTitle, titleConfig, overlayRect.w, overlayRect.h]);
  ...
  useEffect(() => {
    const handleMove = (e) => {
      ...
      if (!d || !onTitleDrag || stageH <= 0) return;
      const s = stageH / 1280;
      const sX = overlayRect.w / 720;
      const dx = (e.clientX - d.startMouseX) / sX;
      const dy = (e.clientY - d.startMouseY) / s;
      const newX = Math.max(0, Math.min(720, Math.round(d.startPosX + dx)));
      const newY = Math.max(0, Math.min(1280, Math.round(d.startPosY + dy)));
      onTitleDrag(newX, newY);
    };
    const handleUp = () => {
  ...
  }, [onTitleDrag, stageH, overlayRect.w]);
  if (!videoSrc) {
    return jsxRuntimeExports.jsx("div", {
    ...
    children: [
      jsxRuntimeExports.jsxs("div", {
        ...
        children: [
          ...
          jsxRuntimeExports.jsx("canvas", {
            ref: titleCanvasRef,
            className: "smartcut-title-canvas",
            width: Math.round(
              overlayRect.w *
          ...
          showTitle &&
            ...
            (() => {
              const style = titleConfig.style;
              const mainConfig = style.mainTitle ?? {};
              const mainText = titleConfig.mainTitleText || '';
              if (!mainText) return null;
              const scaleX = overlayRect.w / 720;
              const scaleY = overlayRect.h / 1280;
              const sc = Math.min(scaleX, scaleY);
              const mainFontSize = (mainConfig.fontSize ?? 48) * sc;
              const mainFontWeight = mainConfig.fontWeight ?? 400;
              const mainFont = mainConfig.font || '黑体';
              const mainPadH = (mainConfig.bgPaddingH ?? (mainConfig.fontSize ?? 48) * 0.5) * sc;
              const mainPadV = (mainConfig.bgPaddingV ?? (mainConfig.fontSize ?? 48) * 0.5) * sc;
              const measureCanvas = document.createElement("canvas");
              const mCtx = measureCanvas.getContext("2d");
              const mainBreakLen = mainConfig.breakLength;
              const mainLines = mainBreakLen != null && mainText.length >= mainBreakLen
                ? splitTextByBreakLength(mainText, mainBreakLen) : [mainText];
              let mainMaxW = mainText.length * mainFontSize * 0.6;
              if (mCtx) {
                mCtx.font = `${mainFontWeight} ${mainFontSize}px "${mainFont}"`;
                mainMaxW = 0;
                for (const line of mainLines) {
                  const w = mCtx.measureText(line).width;
                  if (w > mainMaxW) mainMaxW = w;
                }
              }
              const mainBoxW = mainMaxW + mainPadH * 2;
              const mainLineH = mainFontSize * 1.2;
              const mainBoxH = mainLines.length * mainLineH + mainPadV * 2;
              let totalH = mainBoxH;
              let totalW = mainBoxW;
              if (style.hasSubTitle && style.subTitle && titleConfig.subTitleText) {
                const subConfig = style.subTitle;
                const subFontSize = (subConfig.fontSize ?? 36) * sc;
                const subFontWeight = subConfig.fontWeight ?? 400;
                const subFont = subConfig.font || '黑体';
                const subPadH = (subConfig.bgPaddingH ?? (subConfig.fontSize ?? 36) * 0.5) * sc;
                const subPadV = (subConfig.bgPaddingV ?? (subConfig.fontSize ?? 36) * 0.5) * sc;
                const subBreakLen = subConfig.breakLength;
                const subText = titleConfig.subTitleText;
                const subLines = subBreakLen != null && subText.length >= subBreakLen
                  ? splitTextByBreakLength(subText, subBreakLen) : [subText];
                let subMaxW = subText.length * subFontSize * 0.6;
                if (mCtx) {
                  mCtx.font = `${subFontWeight} ${subFontSize}px "${subFont}"`;
                  subMaxW = 0;
                ...
                const subBoxW = subMaxW + subPadH * 2;
                const subLineH = subFontSize * 1.2;
                const subBoxH = subLines.length * subLineH + subPadV * 2;
                const subGap = (subConfig.top ?? 0) * sc;
                totalH += subGap + subBoxH;
                totalW = Math.max(totalW, subBoxW);
              }
              const alignV = mainConfig.alignV || 'top';
              const topVal = mainConfig.top ?? 100;
              let posY_css;
              if (titleConfig.customPosY != null) {
                posY_css = titleConfig.customPosY * sc;
              } else if (alignV === 'top') {
                posY_css = topVal * sc;
              } else if (alignV === 'bottom') {
                posY_css = overlayRect.h - topVal * sc - totalH;
              } else {
                posY_css = (overlayRect.h - totalH) / 2;
              }
              let posX_css;
              if (titleConfig.customPosX != null) {
                posX_css = titleConfig.customPosX * scaleX;
              } else {
                const alignH = mainConfig.alignH || 'center';
                if (alignH === 'left') {
                  posX_css = overlayRect.w * 0.05 + totalW / 2;
                } else if (alignH === 'right') {
                  posX_css = overlayRect.w * 0.95 - totalW / 2;
                } else {
                  posX_css = overlayRect.w / 2;
                }
              }
              return jsxRuntimeExports.jsx("div", {
                className: `smartcut-title-drag-handle${titleDragging ? " smartcut-title-drag-active" : ""}`,
                style: {
                  left: `${posX_css - totalW / 2}px`,
                  top: `${posY_css}px`,
                  width: `${totalW}px`,
                  height: `${totalH}px`,
                  cursor: titleDragging ? "grabbing" : "grab",
                },
                onMouseDown: (e) => {
                  e.preventDefault();
                  setTitleDragging(true);
                  const curX = titleConfig.customPosX != null ? titleConfig.customPosX : (() => {
                    const alignH = mainConfig.alignH || 'center';
                    if (alignH === 'left') return Math.round(720 * 0.05 + (totalW / scaleX) / 2);
                    if (alignH === 'right') return Math.round(720 * 0.95 - (totalW / scaleX) / 2);
                    return 360;
                  })();
                  const curY = titleConfig.customPosY != null ? titleConfig.customPosY : (() => {
                    if (alignV === 'top') return topVal;
                    if (alignV === 'bottom') return Math.round(1280 - topVal - totalH / sc);
                    return Math.round((1280 - totalH / sc) / 2);
                  })();
...
function SmartCutModal({ show, onClose }) {
  ...
  useEffect(() => {
    if (show) {
      ...
      if (!sessionInitialized) {
        ...
        console.log("[SmartCutModal] 初始化视频路径为空", { previewBase, processingBase, snapshot: { generatedVideoPath: snapshot.generatedVideoPath, originalVideoPath: snapshot.originalVideoPath, generatedVideoPreview: snapshot.generatedVideoPreview } });
        ...
        if (snapshot.generatedVideoPreview) {
          console.log("[SmartCutModal] 使用generatedVideoPreview:", snapshot.generatedVideoPreview);
          setLocalPreviewUrl(snapshot.generatedVideoPreview);
        } else if (nextPreviewPath) {
          console.log("[SmartCutModal] 加载本地文件URL:", nextPreviewPath);
          window.api
            .getLocalFileUrl(nextPreviewPath)
            .then((res) => {
              console.log("[SmartCutModal] getLocalFileUrl结果:", res);
              if (res.success && res.url) setLocalPreviewUrl(res.url);
            })
            .catch((err) => {
              console.error("[SmartCutModal] getLocalFileUrl错误:", err);
              setLocalPreviewUrl(null);
            });
        } else {
          console.log("[SmartCutModal] 无可用视频路径");
        }
        if (processingBase) {
          window.api
            .getLocalFileUrl(processingBase)
            .then((res) => {
              console.log("[SmartCutModal] baseVideoPreviewUrl结果:", res);
              if (res.success && res.url) setBaseVideoPreviewUrl(res.url);
            })
            .catch(() => {});
        }
        const titleConfig = snapshot.titleEffectConfig;
        const currentBuiltinStyles = useVideoPageStore.getState().builtinTitleStyles || [];
        const matchingBuiltin = titleConfig?.style?.id
          ? currentBuiltinStyles.find((s) => s.id === titleConfig.style.id)
          : null;
        console.log('[SmartCutModal] snapshot titleEffectConfig:', JSON.stringify({
          hasConfig: !!titleConfig,
          styleId: titleConfig?.style?.id,
          mainAlignH: titleConfig?.style?.mainTitle?.alignH,
          subAlignH: titleConfig?.style?.subTitle?.alignH,
          hasSubTitle: titleConfig?.style?.hasSubTitle,
          matchingBuiltinFound: !!matchingBuiltin,
          builtinMainAlignH: matchingBuiltin?.mainTitle?.alignH,
          builtinSubAlignH: matchingBuiltin?.subTitle?.alignH,
        }));
        const defaultMain = {
          font: "黑体",
          fontSize: 48,
        ...
        const builtinMain = matchingBuiltin?.mainTitle || {};
        const builtinSub = matchingBuiltin?.subTitle || {};
        const mergedTitleConfig =
          titleConfig != null
            ? {
                ...
                style: {
                  ...titleConfig.style,
                  ...(matchingBuiltin ? { hasSubTitle: matchingBuiltin.hasSubTitle } : {}),
                  mainTitle: {
                    ...defaultMain,
                    ...builtinMain,
                  ...
                  subTitle:
                    (titleConfig.style?.subTitle != null || matchingBuiltin?.subTitle != null)
                      ? { ...defaultSub, ...builtinSub, ...titleConfig.style?.subTitle }
  ...
  const handleAddTitleClick = useCallback(async () => {
    ...
    const _storeState2 = useVideoPageStore.getState();
    const _selectedId = _storeState2.selectedTitleStyleId;
    const _builtins = _storeState2.builtinTitleStyles || [];
    const _selectedBuiltin = _selectedId ? _builtins.find((s) => s.id === _selectedId) : null;
    const _titleStyle = _selectedBuiltin || DEFAULT_TITLE_CONFIG.style;
    console.log('[SmartCutModal] handleAddTitleClick -> styleId:', _titleStyle.id, 'alignH:', _titleStyle.mainTitle?.alignH);
                            ...
                            jsxRuntimeExports.jsx("div", {
                              ...
                              children: jsxRuntimeExports.jsx(
                                CompositePreview,
                                {
                                  ...
                                  onTitleDrag: (posX, posY) => {
                                    setLocalTitleEffectConfig((prev) =>
                                      prev ? { ...prev, customPosX: posX, customPosY: posY } : prev,
                                    );
                                  },
                                  bgmSelectedBgmId:
...
// 导出智能精剪相关组件供外部使用
export { SmartCutModal, SmartCutTimeline, SmartCutResourcePanel, CompositePreview, SmartCutMixUploadWizard }